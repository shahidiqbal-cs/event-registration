    ...
    <div class="box box-primary">
        ...
        <div class="box-body">
            ...
            <div class="row to-right">
                <div class="col-sm-12 pull-right">
                    <div class="form-group">
                        <label class="col-sm-2 control-label pull-right text-left" for="tabiat"><?= lang('event_for') . ' ' . lang('tarbiat'); ?>: </label>
                        <div class="col-sm-10">
                            <div class="row">
                                <div class="col-sm-3 pull-right">
                                    <label class="control-label">
                                        <input type="radio" name="tabiat" class="minimal" value="all" <?= (!$selected_ideology) ? 'checked' : ''; ?> /> <?= lang('all'); ?>
                                    </label>
                                </div>
                                <div class="col-sm-3 pull-right">
                                    <label class="control-label">
                                        <input type="radio" name="tabiat" class="minimal" value="custom" <?= ($selected_ideology) ? 'checked' : ''; ?> /> <?= lang('selected'); ?>
                                    </label>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="tabiat_block" class="<?= (!$selected_ideology)?' hidden':'';?>">
                        <div class="form-group<?= (form_error('ideologies[]')) ? ' has-error' : ''; ?>">
                            <label class="col-sm-2 control-label pull-right text-left" for="ideology_status" ><?= lang('status'); ?>: </label>
                            <div class="col-sm-10">
                                <div class="row">
                                    <?php foreach ($ideologies as $ideology): ?>
                                        <div class="col-sm-3 pull-right">
                                            <input type="checkbox" class="minimal" name="ideologies[]" value="<?= $ideology->ideology_id; ?>" <?= (is_array($selected_ideology) && in_array($ideology->ideology_id, $selected_ideology)) ? 'checked' : ''; ?> />
                        ...
                        <div class="form-group<?= (form_error('majlis_amomis[]')) ? ' has-error' : ''; ?>">
                            <label class="col-sm-2 control-label pull-right text-left" for="majlis_amomis" ><?= lang('majlis_amomi'); ?>: </label>
                            <div class="col-sm-10">
                                <div class="row">
                                    <?php foreach ($majlis_amomis as $majlis_amomi): ?>
                                        <div class="col-sm-3 pull-right">
                                            <input type="checkbox" class="minimal" name="majlis_amomis[]" value="<?= $majlis_amomi->majlis_amomi_id; ?>" <?= (is_array($selected_majlis_amomi) && in_array($majlis_amomi->majlis_amomi_id, $selected_majlis_amomi)) ? 'checked' : ''; ?> />
                                            <label><?= $majlis_amomi->majlis_amomi_status; ?></label>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                            <label id="ideology_message" class="col-sm-12 to-left" for="majlis_amomis"> <?= form_error('majlis_amomis[]'); ?></label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row to-right">
                <div class="col-sm-12 pull-right">
                    <div class="form-group">
                        <label class="col-sm-2 control-label pull-right text-left" for="tabiat"><?= lang('event_for') . ' ' . lang('dawat'); ?>: </label>
                        <div class="col-sm-10">
                            <div class="row">
                                <div class="col-sm-3 pull-right">
                                    <label class="control-label">
                                        <input type="radio" name="dawat" class="minimal" value="all" <?= (!$selected_propagation) ? 'checked' : ''; ?>  /> <?= lang('all'); ?>
                                    </label>
                                </div>
                                <div class="col-sm-3 pull-right">
                                    <label class="control-label">
                                        <input type="radio" name="dawat" class="minimal" value="custom" <?= ($selected_propagation) ? 'checked' : ''; ?> /> <?= lang('selected'); ?>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div id="dawat_block" class="form-group<?= (form_error('propagations[]')) ? ' has-error' : ''; ?><?= (!$selected_propagation) ? ' hidden' : ''; ?>">
                        <label class="col-sm-2 control-label pull-right text-left" for="propagations" ><?= lang('status'); ?>: </label>
                        <div class="col-sm-10">
                            <div class="row">
                                <?php foreach ($propagations as $propagation): ?>
                                    <div class="col-sm-3 pull-right">
                                        <input type="checkbox" class="minimal" name="propagations[]" value="<?= $propagation->propagation_id; ?>" <?= (is_array($selected_propagation) && in_array($propagation->propagation_id, $selected_propagation)) ? 'checked' : ''; ?>/>
            ...
            <hr>
            <div class="row to-right">
                <div class="col-sm-12 pull-right"><label class="col-sm-2 control-label pull-right text-left" ><?= lang('event_for') . ' ' . lang('organization'); ?>: </label></div>
            </div>
            <div id="zone_area">
                <div class="row to-right">
                    <div class="col-sm-12 pull-right">
