    ...
    <form action="" method="post">
        ...
        <div class="box-body to-right">
            <div class="form-group<?= (form_error('organization_name')) ? ' has-error' : ''; ?>">
                <label><?= lang('name') . ' ' . lang('halqa'); ?> :</label>
                <input class="form-control" placeholder="<?= lang('name') . ' ' . lang('halqa'); ?>" name="organization_name" value="<?= $organization_name; ?>" />
                <label class="control-label to-left" for="organization_name"><?= form_error('organization_name'); ?></label>
            </div>
            <div class="form-group<?= (form_error('organization_parent')) ? ' has-error' : ''; ?>">
                <label><?= lang('name') . ' ' . lang('city'); ?> :</label>
                <select class="form-control" name="organization_parent">
                    <?php foreach ($cites as $city): ?>
                        <option value="<?= $city->city_id; ?>" <?= ($city->city_id == $organization_parent) ? 'selected' : ''; ?>><?= $city->city_name; ?></option>
                    <?php endforeach; ?>
                </select>
                <label class="control-label to-left" for="organization_parent"><?= form_error('organization_parent'); ?></label>
            </div>
