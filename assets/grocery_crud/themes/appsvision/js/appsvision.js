...
$(document).ready(function () {
    ...
    $('table.appsvisionTable thead tr th').each(function (index) {
    ...
    if ($('.appsvisionTable').length > 1) {
        use_storage = false;
    }

    $('.appsvisionTable').each(function (index) {
    ...
    $(".appsvisionTable tfoot input").keyup(function () {

        chosen_table = datatables_get_chosen_table($(this).closest('.appsvisionTable'));
    ...
    if (search_values !== null)
    {
        $.each($.parseJSON(search_values), function (num, val) {
            if (val !== '')
            {
                $(".appsvisionTable tfoot tr th:eq(" + num + ")").children(':first').val(val);
            }
        });
    }
    ...
    $('.clear-filtering').click(function () {
        ...
        $(this).closest('.appsvisionTable').find("tfoot tr th input").val("");
    });
    ...
    $('.refresh-data').click(function () {
        ...
        $.ajax({
            ...
            success: function (my_output) {
                ...
                loadDataTable(new_container.find('.appsvisionTable'));
...
function loadDataTable(this_datatables) {
    ...
    return $(this_datatables).dataTable({
        ...
        "oTableTools": {
            "aButtons": aButtons,
            "sSwfPath": base_url + "assets/grocery_crud/themes/appsvision/extras/TableTools/media/swf/copy_csv_xls_pdf.swf"
        }
    });
}
        ...
        $.ajax({
            ...
            success: function (data)
            {
                if (data.success)
                {
                    ...
                    chosen_table = datatables_get_chosen_table($('tr#row-' + row_id).closest('.appsvisionTable'));
